...
export async function MovementsList({
  ...
  const movements = await prisma.transaction.findMany({
    where: {
      ...(filterTruck ? { truckId: filterTruck.id } : {}),
      ...(endedOnly ? { status: "ENDED" as const } : {}),
    },
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-800 mt-1">
            {endedOnly ? t.movements.closedTitle : t.movements.title}
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={!endedOnly ? "default" : "outline"} asChild>
            <Link href="/movements">{t.movements.filterAll}</Link>
          </Button>
          <Button variant={endedOnly ? "default" : "outline"} asChild>
            <Link href="/movements?status=ended">{t.movements.filterClosed}</Link>
          </Button>
          <Button asChild>
      ...
      {movements.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border bg-card p-10 text-center">
          <p className="font-600">
            {endedOnly ? t.movements.closedEmptyTitle : t.movements.emptyTitle}
          </p>
          <p className="text-sm text-muted-foreground mt-1">
            {endedOnly ? t.movements.closedEmptyBody : t.movements.emptyBody}
          </p>
          {!endedOnly && (
